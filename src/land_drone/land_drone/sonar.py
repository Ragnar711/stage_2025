...
THRESHOLD_OBSTACLE_DISTANCE = 10
...
class Sonar(Node):
    def __init__(self, node_name, hostname, auSonarDrv):
        ...
        self.get_logger().info('Sonar node initialised with the topic: {}'.format(self.topic_name))
    ...
    def publish_distance_message(self, distance):
        ...
        self.get_logger().info('Message published on the topic {}: "{}"'.format(self.topic_name, distance))
...
def main(args=None):
    ...
    node = Node('sonar', allow_undeclared_parameters=True, automatically_declare_parameters_from_overrides=True)
