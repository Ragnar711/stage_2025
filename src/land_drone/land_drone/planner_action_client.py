...
class PlannerActionClient(Node):
    ...
    def feedback_callback(self, feedback_handle):
        ...
        if not self.goal_displayed:
            goal_position_str = "Goal Position: [{:.3f}, {:.3f}, {:.3f}]".format(
                self.mfGoalPos[0], self.mfGoalPos[1], self.mfGoalPos[2]
            )
        ...
        self.get_logger().info(
            f"Received feedback : Distance to goal : {distance:.2f} meters"
        )
    ...
    def get_result_callback(self, future):
        ...
        if status == GoalStatus.STATUS_SUCCEEDED:
            self.get_logger().info(
                "Goal succeeded! Final position: {0}".format(result.final_position)
            )
