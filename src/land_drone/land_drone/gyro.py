...
class Gyro(Node):
    def __init__(self, node_name, hostname, auGyroDrv):
        ...
        self.get_logger().info(
            "Nœud gyro initialisé avec le topic : {}".format(self.topic_name)
        )
    ...
    def publish_gyro_message(self, x, y, z, ax, ay, az):
        ...
        gyro_msg.data = [
            float(value) for value in json.dumps(data_dict).encode("utf-8")
        ]
        self.publisher_.publish(gyro_msg)
        self.get_logger().info(
            "Message publié sur le topic {} : {}".format(self.topic_name, data_dict)
        )
