...
class DronePositionControl(Node):
    def __init__(self):
        super().__init__(
            "drone1_position_control"
        )  # Initialise le nœud avec un nom unique
        self.subscription = self.create_subscription(
            Float64, "sonar_data", self.sonar_callback, 10
        )
    ...
    def sonar_callback(
        self, msg
    ):  # Le rappel (callback) appelé lorsque de nouvelles données du sonar sont reçues
        distance = msg.data  # Récupère la distance du sonar depuis le message reçu
        cpm_msg = self.create_cpm_message(
            distance
        )  # Crée un message CPM en fonction de la distance
    ...
    def create_cpm_message(self, distance):
        ...
        cpm_msg = CPM()
        cpm_msg.its_header.generation_time = (
            0  # Temps de génération du message (à remplacer par la valeur réelle)
        )
        cpm_msg.current_position = (
            []
        )  # Position actuelle du drone (à remplir avec les coordonnées réelles)

        if distance < SEUIL_DISTANCE_OBSTACLE:
            cpm_msg.sensor_information = (
                "Obstacle détecté"  # Message indiquant la détection d'un obstacle
            )
        else:
            cpm_msg.sensor_information = (
                "Aucun obstacle"  # Message indiquant l'absence d'obstacle
            )

        cpm_msg.perceive_object = (
            self.sonar_drv.get_distance()
        )  # Récupère la distance depuis le SonarDrv
