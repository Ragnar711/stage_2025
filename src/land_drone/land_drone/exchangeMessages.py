import rclpy  # type: ignore # ROS client library for Python
from rclpy.node import Node  # type: ignore # Base class for creating ROS nodes
from aiv_interfaces.msg import (
    CAM,
    CPM,
    PerceiveObjectContainer,
)  # Message type for CAM (Computer-Aided Manufacturing) data
from std_msgs.msg import Float64, String, Float64MultiArray  # type: ignore
...
class ExchangeMessages(Node):
    def __init__(self, node_name, hostname, neighbors):
        ...
        # MDK - Lets hide this for some time
        # self.subscribe_to_neighbor()

        self.subscription_sonar = self.create_subscription(
            Float64, f"{self.hostname}/sonar_data", self.sonar_data_callback, 10
        )

        self.subscription_gps = self.create_subscription(
            String, f"{self.hostname}/gps_data", self.gps_data_callback, 10
        )

        self.subscription_gyro = self.create_subscription(
            Float64MultiArray, f"{self.hostname}/gyro_data", self.gyro_data_callback, 10
        )
        ...
        for neighbor in self.neighbors:
            if neighbor != self.hostname:
                ...
                subscription = self.create_subscription(
                    CAM, f"/{neighbor}/CAM", self.receive_callback, 10
                )
    ...
    def receive_callback(self, msg):
        ...
        # Log the received message along with the source neighbor
        self.get_logger().info(
            f"Received message from {self.neighbors[0]}/CAM: {msg.current_position}"
        )
    ...
    def gps_data_callback(self, msg):
        ...
        self.get_logger().info(
            f"Received gps data : Latitude: {lfLatitude}, Longitude: {lfLongitude}, Altitude: {lfAltitude} meters"
        )
    ...
    def gyro_data_callback(self, msg):
        ...
        if len(coordonne_gyro) == 6:
            ...
            self.get_logger().warn(
                "Received invalid gyro data: {}".format(coordonne_gyro)
            )
