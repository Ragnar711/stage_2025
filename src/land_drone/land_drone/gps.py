...
class GPS(Node):
    def __init__(self, node_name, hostname, auGPSDrv):
        ...
        self.get_logger().info('GPS node initialised with topic: {}'.format(self.topic_name))
    ...
    def publish_gps_message(self,coordinates):
        ...
        self.get_logger().info('Message published on the topic {}: {}'.format(self.topic_name, coordinates))
...
def main(args=None):
    ...
    node = Node('gps', allow_undeclared_parameters=True, automatically_declare_parameters_from_overrides=True)
