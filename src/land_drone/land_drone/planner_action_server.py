...
class PlannerActionServer(Node):
    ...
    def __init__(self, robot_name):
        ...
        self.muPubGoalPos = self.create_publisher(luMsgType, self.msTopicName, luQos)
        ...
        self.get_logger().info(
            "Terrestrial drone planner action server has been initialized."
        )
    ...
    async def execute_callback(self, goal_handle):
        ...
        luCartesianGoalPos = Position3D(
            goal_position[0], goal_position[1], goal_position[2]
        )
        self.get_logger().info(
            "Executing goal to move until: {:.3f}, {:.3f}, {:.3f}".format(
                *goal_position
            )
        )
        ...
        while True:
            ...
            lsGPSCoordinates = (
                self.luGPSDrv.get_coordinates()
            )  # Assuming this gets the current position
            lfLatitude, lfLongitude, lfAltitude = self.extract_coordinates(
                lsGPSCoordinates
            )
            ...
            luCartesianCurPos = Position3D.fromGPScoordinates(
                lfLatitude, lfLongitude, lfAltitude
            )
            ...
            self.get_logger().info(
                f"Cartesian current position : {luCartesianCurPos.toString()}"
            )
            ...
            self.get_logger().info(
                f"Feedback sent to client: Current Position: latitude = {feedback_msg.current_position[0]:.3f}, longitude = {feedback_msg.current_position[1]:.3f}, altitude = {feedback_msg.current_position[2]:.3f}"
            )
            ...
            if distance < 0.1:  # You can adjust this threshold as needed
                ...
                result_msg.final_position = [
                    luCartesianGoalPos.x,
                    luCartesianGoalPos.y,
                    luCartesianGoalPos.z,
                ]
                self.get_logger().info(
                    f"Goal succeeded! Final position : [{result_msg.final_position[0]:.3f}, {result_msg.final_position[1]:.3f}, {result_msg.final_position[2]:.3f}]"
                )
                return result_msg
