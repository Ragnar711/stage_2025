...
class PositionControl(Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(
            luMsgType, lsTopicName, self.cam_response_callback, qos
        )

        luMsgType = CPM
        lsTopicName = self.robot_name + "/CPM"
        self.create_subscription(
            luMsgType, lsTopicName, self.cpm_response_callback, qos
        )

        luMsgType = Vector2Goal
        lsTopicName = self.robot_name + "/Vector2Goal"
        self.create_subscription(
            luMsgType, lsTopicName, self.Vector2Goal_response_callback, qos
        )
        ...
        self.get_logger().info(
            "Terrestrial drone position control node has been initialised."
        )
    ...
    def control_loop(self):
        ...
        if distance > 1.0:  # Threshold to advance
            ...
            self.get_logger().info(
                f"Moving forward. Distance to goal: {distance:.2f} meters"
            )
        else:
